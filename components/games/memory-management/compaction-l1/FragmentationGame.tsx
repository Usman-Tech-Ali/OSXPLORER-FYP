...
class LevelSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    const levels = [
      {
        key: 'FragL1Scene',
        title: 'Level 1: Fixed Partitions',
        subtitle: 'Internal Fragmentation',
        desc: 'Items snap into fixed large crates.\nWasted space inside = internal fragmentation.\nWatch the waste meter fill up.',
        color: '#FFD700',
        y: 0.36,
      },
      {
        key: 'FragL2Scene',
        title: 'Level 2: Dynamic Partitions',
        subtitle: 'External Fragmentation',
        desc: 'Items shrink-wrapped to exact size.\nDrones remove middle blocks.\nLarge server can\'t fit in scattered gaps.',
        color: '#FF6666',
        y: 0.56,
      },
      {
...
export default function FragmentationGame({ level }: Props = {}) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !gameRef.current) {
      ...
      // pick start scene
      const startScene =
        level === 'l1' ? 'FragL1Scene' :
        level === 'l2' ? 'FragL2Scene' :
        level === 'l3' ? 'FragL3Scene' :
        'LevelSelectScene';
      ...
      gameRef.current = new Phaser.Game({
        ...
        scene: [LevelSelectScene, FragL1Scene],
      });

      // jump to requested level after boot
      gameRef.current.events.once('ready', () => {
        if (startScene !== 'LevelSelectScene') {
