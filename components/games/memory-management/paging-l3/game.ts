...
export class PagingL3Game extends Phaser.Scene {
  ...
  create() {
    ...
    // L3: Thrashing – 4 gates need 4 different gadgets; belt has only 3 slots (must replace)
    this.gates = [
      { x: width * 0.35, y: height * 0.5, required: 'drill', label: 'Drill', passed: false },
      { x: width * 0.5, y: height * 0.38, required: 'laser', label: 'Laser', passed: false },
      { x: width * 0.65, y: height * 0.5, required: 'card', label: 'Keycard', passed: false },
      { x: width * 0.5, y: height * 0.26, required: 'computer', label: 'Hacker', passed: false },
    ];
    ...
    this.instructionText = this.add.text(width / 2, height - 28, 'Heist Protocol L3: Thrashing. You must drop something to make room!', {
  ...
  private showIntro(width: number, height: number) {
    ...
    const title = this.add.text(width / 2, height * 0.32, 'Heist Protocol – Level 3', {
      fontSize: '28px', color: '#FFD700', fontStyle: 'bold',
    }).setOrigin(0.5).setDepth(31);
    const sub = this.add.text(width / 2, height * 0.42, 'The Mastermind: Thrashing & Replacement', {
      fontSize: '18px', color: '#fff',
    }).setOrigin(0.5).setDepth(31);
    const body = this.add.text(width / 2, height * 0.52,
      'Belt has 3 slots but you need 4 different gadgets.\nYou MUST drop one to pick another.\nChoose wisely – if the next gate needs what you dropped, you thrash (run back again)!',
    ...
    btn.on('pointerdown', () => {
      ...
      this.phaseText.setText('Phase: Playing (Thrashing)');
      this.instructionText.setText('1) Move to VAN (bottom) with WASD/Arrows  2) Press E to get gadgets (drop one if belt full)  3) Walk into each gate to open it.');
  ...
  private tryOpenGate(gate: GateConfig) {
    ...
    if (hasIt) {
      ...
      this.instructionText.setText('Page fault! You need ' + gate.label + '. Run to the van – if belt is full, drop something first.');
    }
  }
  ...
  private win() {
    ...
    this.instructionText.setText('Heist complete! Page faults: ' + this.pageFaults + ' (minimize by smart replacement).');
    this.submitScore();
  ...
  private async submitScore() {
    try {
      ...
      console.error('Failed to submit paging L3 score:', error);
    }
  }
