...
export class FCFSGameL2 extends Phaser.Scene {
  // Game State
  ...
  // Timing
  private numCustomers: number = 0; // Will be randomly set (4-6) for Medium difficulty
  ...
  private startArrivalPhase() {
    ...
    // Randomly decide how many customers (4 to 6) for Medium difficulty
    this.numCustomers = Phaser.Math.Between(4, 6);
    ...
    // Create all customers with staggered arrival times (L2 Medium difficulty)
    this.createAllCustomers();
    
    // After showing customers, start waiter collecting orders
    // For L2, customers arrive at different times, so we delay waiter start slightly
  ...
  private createAllCustomers() {
    ...
    for (let customerIndex = 0; customerIndex < this.numCustomers; customerIndex++) {
      ...
      // Create order with random dish (more randomness for Medium)
      const dishType = dishTypes[Phaser.Math.Between(0, dishTypes.length - 1)];
      const dishConfig = this.DISH_CONFIGS[dishType];
      
      // Add variation to burst time (±1 second) for Medium difficulty
      const burstTimeVariation = Phaser.Math.Between(-1, 1);
      const adjustedBurstTime = Math.max(2, dishConfig.burstTime + burstTimeVariation);
      
      const order: FoodOrder = {
        id: `order-${customerIndex + 1}`,
        orderNumber: 0, // Will be assigned when waiter visits
        customerName: name,
        arrivalTime: 0, // Will be set when waiter visits (arrival times vary naturally)
        burstTime: adjustedBurstTime,
        dishType: dishType,
        isCompleted: false,
        isDelivered: false,
        isCooking: false
  ...
  private collectOrderFromCustomer(customer: Customer, orderNumber: number) {
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        ...
        // Set arrival time to current game time (or use pre-set staggered arrival time)
        // For L2, arrival time was set during customer creation with staggered times
        if (customer.order.arrivalTime === 0) {
