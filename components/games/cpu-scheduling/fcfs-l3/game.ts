...
export class FCFSGameL3 extends Phaser.Scene {
  // Game State
  ...
  // Game Data - Extended names for Hard difficulty
  private readonly CUSTOMER_NAMES = ['Alice', 'Bob', 'Carol', 'Dave', 'Emma', 'Frank', 'Grace', 'Henry'];
  ...
  // Timing
  private numCustomers: number = 0; // Will be randomly set (5-8) for Hard difficulty
  ...
  private startArrivalPhase() {
    ...
    // Randomly decide how many customers (5 to 8) for Hard difficulty
    this.numCustomers = Phaser.Math.Between(5, 8);
  ...
  private createAllCustomers() {
    ...
    for (let customerIndex = 0; customerIndex < this.numCustomers; customerIndex++) {
      ...
      // Create order with random dish (high randomness for Hard)
      const dishType = dishTypes[Phaser.Math.Between(0, dishTypes.length - 1)];
      const dishConfig = this.DISH_CONFIGS[dishType];
      
      // Add significant variation to burst time (±2 seconds) for Hard difficulty
      const burstTimeVariation = Phaser.Math.Between(-2, 2);
      const adjustedBurstTime = Math.max(2, Math.min(10, dishConfig.burstTime + burstTimeVariation));
