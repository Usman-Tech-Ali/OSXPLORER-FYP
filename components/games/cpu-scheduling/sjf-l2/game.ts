...
export class SJFGameL2 extends Phaser.Scene {
  ...
  private numPeople: number = 0; // Will be randomly set (4-6) for Medium difficulty
  ...
  private startArrivalPhase() {
    ...
    // Medium difficulty: 4-6 people
    this.numPeople = Phaser.Math.Between(4, 6);
  ...
  private createAllPeople() {
    ...
    for (let i = 0; i < this.numPeople; i++) {
      ...
      // More randomness in file selection for Medium difficulty
      const fileSize = fileSizes[Phaser.Math.Between(0, fileSizes.length - 1)];
      const fileConfig = this.FILE_CONFIGS[fileSize];
      
      // Add variation to burst time (Â±1 second) for Medium difficulty
      const burstTimeVariation = Phaser.Math.Between(-1, 1);
      const adjustedBurstTime = Math.max(2, Math.min(9, fileConfig.burstTime + burstTimeVariation));
      
      // More varied arrival times (0-4 seconds) for Medium difficulty
      const arrivalTime = Phaser.Math.Between(0, 4);
      ...
      const request: FileRequest = {
        ...
        requestNumber: i + 1,
        personName: name,
        arrivalTime: arrivalTime, // Varied arrival times
        burstTime: adjustedBurstTime, // Varied burst times
        fileSize: fileSize,
        isCompleted: false,
        isProcessing: false
      };
  ...
  private onPersonClick(person: Person) {
    ...
    // For L2, preserve pre-set varied arrival time (already set during creation)
    // The varied arrival times (0-4 seconds) represent when they actually arrive
