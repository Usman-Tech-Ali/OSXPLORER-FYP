...
export class SRTFGameL2 extends Phaser.Scene {
  ...
  // Extended names for Medium difficulty
  private readonly PATIENT_NAMES = ['Alice', 'Bob', 'Carol', 'Dave', 'Emma', 'Frank', 'Grace', 'Henry'];
  
  private numPatients: number = 0; // Will be randomly set (7-8) for Medium difficulty
  ...
  private startArrivalPhase() {
    ...
    // Medium difficulty: 7-8 patients
    this.numPatients = Phaser.Math.Between(7, 8);
  ...
  private preparePatients() {
    ...
    // Medium difficulty: More patients with varied burst times and arrival times
    // Base burst times with variation (Â±1-2 seconds)
    const baseBurstTimes = [15, 6, 10, 3, 8, 5, 7, 4];
    const patientNames = ['Alice', 'Bob', 'Carol', 'Dave', 'Emma', 'Frank', 'Grace', 'Henry'];
    ...
    for (let i = 0; i < this.numPatients; i++) {
      ...
      // Add variation to burst time (Â±1-2 seconds) for Medium difficulty
      const baseBurstTime = baseBurstTimes[i % baseBurstTimes.length];
      const burstTimeVariation = Phaser.Math.Between(-2, 2);
      const burstTime = Math.max(2, Math.min(18, baseBurstTime + burstTimeVariation));
      
      // More varied arrival times (0-5 seconds apart) for Medium difficulty
      const baseArrivalTime = i === 0 ? 0 : Phaser.Math.Between(2, 5) + (i - 1) * 3;
      const arrivalTime = i === 0 ? 0 : baseArrivalTime + Phaser.Math.Between(-1, 2);
