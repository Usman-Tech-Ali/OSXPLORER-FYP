...
const QUANTUM = 3;
const CONTEXT_SWITCH_DELAY_MS = 1500;

export class RoundRobinL2Game extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('sweat', `${p}Sweat.png`);
  ...
  private showIntro(width: number, height: number) {
    ...
    const textGoal = this.add.text(boxX + 40, contentY + 143,
      'Clear the queue. Notice how context switch overhead affects throughput.', {
  ...
  private showContextSwitchThenDispatch() {
    const { width, height } = this.sys.game.canvas;
    const sweatX = width * 0.5;
    const sweatY = height * 0.35;
    const sweatSprite = this.add.sprite(sweatX, sweatY, 'sweat');
    sweatSprite.setScale(0.5);
    sweatSprite.setDepth(25);
    this.instructionText.setText('Context switch... (1.5s delay)');
    this.time.delayedCall(CONTEXT_SWITCH_DELAY_MS, () => {
      sweatSprite.destroy();
      this.dispatchNext();
    });
  }
  ...
  private dispatchNext() {
    ...
    if (next.container && next.sprite) {
      ...
      this.instructionText.setText(`Click to shoot! ${next.name} has ${QUANTUM} balls.`);
      this.tweens.add({
  ...
  private showBallAnimation(player: Player) {
    ...
    const targetY = this.hoopSprite.y + 22;
    this.tweens.add({
      targets: ball,
      x: targetX,
      y: targetY,
  ...
  private updatePlayerLabel(player: Player) {
    if (player.labelText) {
      player.labelText.setText(`${player.name} (left: ${player.remainingScore}, balls: ${player.ballsInRack})`);
    }
  }
  ...
  private finishPlayer(player: Player) {
    ...
    if (player.container && player.sprite) {
      ...
      this.instructionText.setText(`${player.name} finished! Leaving court...`);
      this.tweens.add({
        ...
        onComplete: () => {
          ...
          this.showContextSwitchThenDispatch();
        },
      });
    } else {
      this.showContextSwitchThenDispatch();
  ...
  private preemptPlayer(player: Player) {
    ...
    if (player.container && player.sprite) {
      ...
      this.tweens.add({
        targets: player.container,
        x: queueStartX + newIndex * spacing,
        y: queueY,
        duration: 700,
        ease: 'Power2',
        onComplete: () => {
    ...
    try {
      await fetch('/api/score', {
        ...
        body: JSON.stringify({
          ...
          metadata: { playersFinished: this.finishedPlayers.length, totalPlayers: this.players.length },
        }),
      });
    } catch (e) {
...
export const RoundRobinL2GameConfig = { type: Phaser.AUTO, scene: RoundRobinL2Game };
