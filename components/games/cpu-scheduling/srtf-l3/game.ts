...
export class SRTFGameL3 extends Phaser.Scene {
  ...
  private readonly PATIENT_NAMES = ['Alice', 'Bob', 'Carol', 'Dave', 'Emma', 'Frank', 'Grace', 'Henry', 'Ivy', 'Jack'];
  
  private numPatients: number = 0; // Will be randomly set (8-10) for Hard difficulty
  ...
  private startArrivalPhase() {
    ...
    // Hard difficulty: 8-10 patients
    this.numPatients = Phaser.Math.Between(8, 10);
  ...
  private preparePatients() {
    ...
    // Hard difficulty: Many patients with highly varied burst times and arrival times
    // Base burst times with significant variation (Â±2-3 seconds)
    const baseBurstTimes = [15, 6, 10, 3, 8, 5, 7, 4, 12, 9];
    const patientNames = ['Alice', 'Bob', 'Carol', 'Dave', 'Emma', 'Frank', 'Grace', 'Henry', 'Ivy', 'Jack'];
    ...
    for (let i = 0; i < this.numPatients; i++) {
      ...
      // Significant variation to burst time (Â±2-3 seconds) for Hard difficulty
      const baseBurstTime = baseBurstTimes[i % baseBurstTimes.length];
      const burstTimeVariation = Phaser.Math.Between(-3, 3);
      const burstTime = Math.max(2, Math.min(20, baseBurstTime + burstTimeVariation));
      
      // Highly varied arrival times (0-8 seconds apart) for Hard difficulty
      const baseArrivalTime = i === 0 ? 0 : Phaser.Math.Between(1, 4) + (i - 1) * 2;
      const arrivalTime = i === 0 ? 0 : baseArrivalTime + Phaser.Math.Between(-2, 3);
      ...
      const patient: Patient = {
        ...
        patientNumber: i + 1,
        name: patientNames[i],
        arrivalTime: Math.max(0, arrivalTime), // Ensure non-negative
