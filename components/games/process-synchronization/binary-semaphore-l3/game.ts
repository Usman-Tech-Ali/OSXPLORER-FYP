...
🎯 Goal: Pass 10 cars safely!`;
  ...
  private startGame() {
    ...
    // L3: Spawn cars much more frequently with high variation
    this.spawnEvent = this.time.addEvent({
      delay: Phaser.Math.Between(3000, 4000), // Much faster spawning, with high variation
      callback: () => {
  ...
  private moveCar(car: Car) {
    ...
    this.tweens.add({
      targets: car.sprite,
      x: waitingX,
      duration: Phaser.Math.Between(1400, 1700), // L3: Much faster with high variation
      ease: 'Linear',
      onComplete: () => {
        car.isWaiting = true;
