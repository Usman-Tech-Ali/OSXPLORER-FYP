...
🎯 Goal: Pass 10 cars safely!`;
  ...
  private startGame() {
    ...
    // L2: Spawn cars more frequently with variation
    this.spawnEvent = this.time.addEvent({
      delay: Phaser.Math.Between(4500, 5500), // Faster spawning, with variation
      callback: () => {
  ...
  private moveCar(car: Car) {
    ...
    this.tweens.add({
      targets: car.sprite,
      x: waitingX,
      duration: Phaser.Math.Between(1700, 1900), // L2: Slightly faster with variation
      ease: 'Linear',
      onComplete: () => {
        car.isWaiting = true;
